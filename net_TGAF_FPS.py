...
class TGDA_7(nn.Module):
    def __init__(self):
        ...
        self.inputconv = nn.Sequential(nn.Conv2d(1 * 3, 64, 3, padding=3//2),
            nn.LeakyReLU(negative_slope=0.1, inplace=True))
        self.Unet = Unet(
            in_nc=64,   
            out_nc=64, 
        ...
        # # self.offset_mask_w = nn.Conv2d(
        # #     opts_dict['stdf']['nf'], 3*3*1**2, 3, padding=3//2)
        
        # self.deform_conv_w = ModulatedDeformConv(
        #     opts_dict['stdf']['out_nc'],
        #     opts_dict['stdf']['out_nc'], 1, 
        #     padding=1//2, deformable_groups=opts_dict['stdf']['out_nc']
        #     )
       
        self.RCAB = RCAB(64)
        self.fuse = nn.Sequential(
            nn.Conv2d(64*2, 64, 3, stride=1, padding=3//2),
            nn.LeakyReLU(negative_slope=0.1, inplace=True),
        )
...
if __name__ == "__main__":
    ...
    # opts_dict = '/home/zhuqiang/STDF30/config/TGDA/option_TGDA_7_37.yml'
    net = TGDA_7().cuda()
    from thop import profile
    with torch.no_grad():

        input = torch.randn(1, 7, 416, 240).cuda()
