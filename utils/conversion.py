...
def paired_random_crop_prior(img_gts, img_lqs, img_preds, img_PAIs, img_mvs, img_residues, gt_patch_size, gt_path, scale=1):
    """Paired random crop.
    It crops lists of lq and gt images with corresponding locations.
    Args:
        img_gts (list[ndarray] | ndarray): GT images. Note that all images
    ...
    # crop lq patch
    # print('[img_mvs]',img_lqs[0].shape, img_PAIs[0].shape, img_mvs[0].shape, img_residues[0].shape)
    img_lqs = [ v[top:top + lq_patch_size, left:left + lq_patch_size, ...] for v in img_lqs ]
    img_preds = [ v[top:top + lq_patch_size, left:left + lq_patch_size, ...] for v in img_preds ]
    img_PAIs = [ v[top:top + lq_patch_size, left:left + lq_patch_size, ...] for v in img_PAIs ]
    img_mvs = [ v[top:top + lq_patch_size, left:left + lq_patch_size, :] for v in img_mvs ]
    img_residues = [ v[top:top + lq_patch_size, left:left + lq_patch_size, ...] for v in img_residues ]
    ...
    if len(img_lqs) == 1:
        img_lqs = img_lqs[0]
    return img_gts, img_lqs, img_preds, img_PAIs, img_mvs, img_residues
...
def augment(imgs, hflip=True, rotation=True, flows=None):
    ...
    def _augment_flow(flow):
        ...
        flow_t = flow.copy()
        if hflip:
            _imflip_(flow_t, 'horizontal')
            flow_t[:, :, 0] *= -1
        if vflip:
            _imflip_(flow_t, 'vertical')
            flow_t[:, :, 1] *= -1
        if rot90:
            flow_t = flow_t.transpose(1, 0, 2)
            flow_t = flow_t[:, :, [1, 0]]
        return flow_t
